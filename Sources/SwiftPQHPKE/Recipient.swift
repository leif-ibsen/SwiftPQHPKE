//
//  HPKE.swift
//  SwiftHPKETest
//
//  Created by Leif Ibsen on 19/06/2023.
//

public class Recipient {
    
    let suite: CipherSuite
    let info: Bytes
    var key: Bytes = []
    var base_nonce: Bytes = []
    var nonce: Bytes = []
    var exporter_secret: Bytes = []


    // MARK: Initializers

    /// Creates a Recipient instance in base mode
    ///
    /// - Parameters:
    ///   - suite: The CipherSuite of the Recipient
    ///   - privateKey: The recipient private key
    ///   - info: The additional information
    ///   - encap: The encapsulated key
    /// - Throws: An exception if `privateKey` does not match `suite` or `encap` has wrong size
    public init(suite: CipherSuite, privateKey: PrivateKey, info: Bytes, encap: Bytes) throws {
        try suite.checkPrivKey(privateKey)
        self.suite = suite
        self.info = info
        let sharedSecret = try KEMStruct.decap(encap, privateKey)
        (self.key, self.base_nonce, self.exporter_secret) = self.suite.keySchedule(CipherSuite.BASE, sharedSecret, self.info, [], [])
        self.nonce = Bytes(repeating: 0, count: self.base_nonce.count)
    }

    /// Creates a Recipient instance in preshared key mode
    ///
    /// - Parameters:
    ///   - suite: The CipherSuite of the Recipient
    ///   - privateKey: The recipient private key
    ///   - info: The additional information
    ///   - psk: The preshared key
    ///   - pskId: The preshared key id
    ///   - encap: The encapsulated key
    /// - Throws: An exception if `privateKey` does not match `suite` or `encap` has wrong size
    public init(suite: CipherSuite, privateKey: PrivateKey, info: Bytes, psk: Bytes, pskId: Bytes, encap: Bytes) throws {
        try suite.checkPrivKey(privateKey)
        guard CipherSuite.checkPsk(psk, pskId) else {
            throw HPKEException.pskError
        }
        self.suite = suite
        self.info = info
        let sharedSecret = try KEMStruct.decap(encap, privateKey)
        (self.key, self.base_nonce, self.exporter_secret) = suite.keySchedule(CipherSuite.PSK, sharedSecret, self.info, psk, pskId)
        self.nonce = Bytes(repeating: 0, count: self.base_nonce.count)
    }


    // MARK: Instance Methods

    /// Decrypts a message
    ///
    /// - Parameters:
    ///   - ct: The cipher text to decrypt
    ///   - aad: The associated data
    /// - Returns: The plain text
    /// - Throws: An exception if decryption fails or `self.suite.aead` is EXPORTONLY
    public func open(ct: Bytes, aad: Bytes) throws -> Bytes {
        return try self.suite.aeadStruct.open(self.key, aad, computeNonce() + ct)
    }

    /// Retrieve an export secret
    ///
    /// - Parameters:
    ///   - context: The export context
    ///   - L: The length of the export secret
    /// - Returns: The export secret
    /// - Throws: An exception if L is negative or too large
    public func receiveExport(context: Bytes, L: Int) throws -> Bytes {
        try self.suite.checkExportSize(L)
        return self.suite.kdfStruct.labeledExpand(self.exporter_secret, Bytes("sec".utf8), context, L)
    }

    func computeNonce() -> Bytes {
        var x = self.base_nonce
        for i in 0 ..< self.nonce.count {
            x[i] ^= self.nonce[i]
        }
        for i in (0 ..< self.nonce.count).reversed() {
            self.nonce[i] &+= 1
            if self.nonce[i] > 0 {
                break
            }
        }
        return x
    }

}
