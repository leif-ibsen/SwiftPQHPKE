...
public class Sender {
    ...
    public init(suite: CipherSuite, publicKey: PublicKey, info: Bytes) throws {
        try suite.checkPubKey(publicKey)
        self.suite = suite
        self.info = info
        let (sharedSecret, enc) = KEMStruct.encap(publicKey)
        self.encapsulatedKey = enc
        (self.key, self.base_nonce, self.exporter_secret) = self.suite.keySchedule(CipherSuite.BASE, sharedSecret, self.info, [], [])
    ...
    public init(suite: CipherSuite, publicKey: PublicKey, info: Bytes, psk: Bytes, pskId: Bytes) throws {
        try suite.checkPubKey(publicKey)
        ...
        let (sharedSecret, enc) = KEMStruct.encap(publicKey)
        self.encapsulatedKey = enc
        (self.key, self.base_nonce, self.exporter_secret) = self.suite.keySchedule(CipherSuite.PSK, sharedSecret, self.info, psk, pskId)
        self.nonce = Bytes(repeating: 0, count: self.base_nonce.count)
    }
    ...
    public func seal(pt: Bytes, aad: Bytes) throws -> Bytes {
        return try self.suite.aeadStruct.seal(self.key, computeNonce(), aad, pt)
    }
    
    /// Generate an export secret
    ///
    /// - Parameters:
    ...
    public func sendExport(context: Bytes, L: Int) throws -> Bytes {
        try self.suite.checkExportSize(L)
        return self.suite.kdfStruct.labeledExpand(self.exporter_secret, Bytes("sec".utf8), context, L)
    }
